...
void drawSierpinskiTriangle(float x, float y, double size, int iterations, float self_similarity)
{
    if (iterations <= 0)
    {
        ...
        drawTriangle(x - size / 2, y - size * sqrt(3) / 6, size, size, color);
    }
    else
    {
        double buff = size * self_similarity;
        drawSierpinskiTriangle(x - buff / 2, y - buff * sqrt(3) / 6, buff, iterations - 1, self_similarity);
        drawSierpinskiTriangle(x + buff / 2, y - buff * sqrt(3) / 6, buff, iterations - 1, self_similarity);
        drawSierpinskiTriangle(x, y + buff * sqrt(3) / 2 - buff * sqrt(3) / 6, buff, iterations - 1, self_similarity);
    }
}
