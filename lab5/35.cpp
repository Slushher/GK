...
GLfloat light_specular[4] = {0.0, 1.0, 0.0, 1.0};
const GLfloat light_position[4] = {0.0, 0.0, 10.0, 1.0};
...
void keyboard_key_callback(GLFWwindow* window, int key, int scancode, int action, int mods){
    ...
    if(key == GLFW_KEY_UP && action == GLFW_PRESS){
        if(light_specular[light_mode]>=1.0){
            light_specular[light_mode] = 1.0;
        }
        else{
            light_specular[light_mode]+=0.1;
        }
        printf("R:%f G:%f B:%f A:%f\n",light_specular[0],light_specular[1],light_specular[2],light_specular[3]);
    }
    if(key == GLFW_KEY_DOWN && action == GLFW_PRESS){
        if(light_specular[light_mode]<=0.0){
            light_specular[light_mode] = 0.0;
        }
        else{
            light_specular[light_mode]-=0.1;
        }
